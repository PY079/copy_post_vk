...
from id_db import pos_id, add_text, upd_id
...
def wall_get():
    audio, poll, album, doc, youla, sp_vk, sp_tg = False, False, False, False, False, [], []
    ...
    if 'text' in wa['response']['items'][0]:
        ...
        block_list=['тогда тебе','записывайтесь','ждём вас','ждем вас',
            ...
            'наш адрес', 'наши адреса', 'требуются', 'требуется',
            'записывайся', 'записывайтесь','записывайся по номеру',
    ...
    if 'text' in wa['response']['items'][0] and not 'В посте найдено голосование' in text_tg:
        ...
        if add_text(mes_text) is True:

            if len(mes_text)>3966:
                text_tg+= 'Bad Request: message is too long. Max 4096\n\n-------------\n'
            else:
                text_tg+= f'{mes_text}\n'
            text_in_post = True; repeated_text = False
        else: repeated_text = True
    ...
    if copy_history is False and marked_as_ads is False and block_comments is False and post_ads is False and repeated_text is False:
        if 'attachments' in wa['response']['items'][0]:
            for i in wa['response']['items'][0]['attachments']:
                ...
                if _type == 'photo' or _type =='video':
                    ...
                    if _type == 'video':
                        sp_tg.append(f'https://vk.com/photo{id_gossip_nf_clone}_457239023')
                        sp_vk.append(f"{_type}{i[_type]['owner_id']}_{i[_type]['id']}_{i[_type]['access_key']}")
        ...
        if len(sp_vk)>0 and text_in_post == True and signer_id == True:
            if geo == False:
                ...
                if audio == False and poll == False and album == False and doc == False and youla == False:
                    ...
                    text_tg+= f"<a href='{cop}'>Оригинальный пост</a>\n<a href='https://vk.com/wall{id_gossip_nf}_{id['post_id']}'>Скопированный пост</a>\n\n<a href='https://vk.com/gossipnfclone'>Gossip NF CLONE</a>\n<a href='https://vk.com/gossipnf'>Gossip NF</a>"
